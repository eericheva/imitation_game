...
from setup.setup import CURRENT_USER_ID, CURRENT_USER_USERNAME
...
class Game:
    ...
    def start_game(self, call):
        self.playerA = self.playerA.setup_player(
            id=CURRENT_USER_ID(call), username=CURRENT_USER_USERNAME(call)
        )
        self.playerB = self.playerB.setup_player()
        self.playerC = self.playerC.setup_player()
    ...
    def proceed_user_message(self, call, user_message):
        ...
        if not is_there_decision:
            ...
            telegram_bot_start_session.send_decision(
                self.last_call, self.playerC.decision
            )
